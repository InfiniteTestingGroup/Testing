...
public interface ad_campaignsRepository extends MongoRepository<ad_campaigns, String>, AdCampaignRepositoryCustom {
	...
	@Query("{'createdBy': ?0}")
	List<ad_campaigns> findByCreatedByObjectId(org.bson.types.ObjectId createdBy);

	// @Query(value="{compaignsStatus:'ACTIVE'}",
	// fields="{'location.lat':1,'location.lng':1,'compaignsStatus':1,'createdBy':1,'location.range':1,'dateRange':1,'campaignCategories':1,'advertisementId':1}")
	// @Query(value="{compaignsStatus:'ACTIVE'}")
	...
	// "{$project:{_id:1,compaignsStatus:1,createdBy:1,advertisementId:1,dateRange:1,latitude:'$location.lat',longitude:'$location.lng',range:'$location.range','title':
	// '$advertisementDetails.title','description':'$advertisementDetails.description'}}"
	...
	// @Query(value="{{compaignsStatus:'ACTIVE'},'$or':[{'campaignCategories':{
	// '$in':?0 }},{createdBy:?1}]}")
	...
	// @Query(value="{'$and':[{'location.locationName': {$regex:'.*?0.*',
	// $options:'i'} },{compaignsStatus:'ACTIVE'}]}")
